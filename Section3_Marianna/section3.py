...
plt.plot([1, 48], [P_1, (((1200**3)*48)/Ts )*10**(-6)]) #retta perfect performance
plt.savefig('fig.png')
plt.close()

plt.scatter([4,8,12], P[5:8], label = 'two socket')
plt.scatter([4,8,12], P[8:11], label = 'same socket')
plt.legend()
plt.savefig('socket.png')
plt.close()

plt.scatter([4,8,12], T_c_tot[5:8], label = 'two socket')
plt.scatter([4,8,12], T_c_tot[8:11], label = 'same socket')
plt.legend()
plt.savefig('tc.png')
plt.close
